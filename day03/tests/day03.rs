...
fn test_extract_mul_instructions_example() {
    let instructions = extract_mul_instructions(EXAMPLE_INPUT);
...
fn test_extract_mul_instructions_edge_cases() {
    ...
    // Test empty input
    assert_eq!(extract_mul_instructions(""), vec![]);

    // Test no valid instructions
    assert_eq!(extract_mul_instructions("no mul instructions here"), vec![]);

    // Test boundary cases for 1-3 digits
    let memory2 = "mul(1,2) mul(12,34) mul(123,456) mul(1234,5) mul(1,2345)";
    let instructions2 = extract_mul_instructions(memory2);
...
fn test_solve_part1_simple() {
    assert_eq!(solve_part1("mul(2,3)"), 6);
    assert_eq!(solve_part1("mul(10,10)"), 100);
    assert_eq!(solve_part1("mul(1,1)mul(2,2)mul(3,3)"), 14); // 1 + 4 + 9 = 14
}

#[test]
fn test_solve_part1_no_valid_instructions() {
    assert_eq!(solve_part1("no valid instructions"), 0);
    assert_eq!(solve_part1("mul(4* mul[3,7] mul ( 2 , 4 )"), 0);
}
