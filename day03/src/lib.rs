...
///
/// Uses regex pattern `mul\((\d{1,3}),(\d{1,3})\)` to find instructions with the
/// exact format mul(X,Y) where X and Y are 1-3 digit numbers. Invalid formats
/// like mul(4*, mul[3,7], or mul ( 2 , 4 ) are ignored.
///
/// # Parameters
/// * `input` - String containing corrupted memory with mixed valid/invalid instructions
///
/// # Returns
...
///
/// # Parameters
/// * `input` - String containing corrupted memory with mul, do(), and don't() instructions
///
/// # Returns
...
pub fn extract_enabled_mul_instructions(input: &str) -> Result<Vec<(u32, u32)>> {
    ...
    for captures in RE.captures_iter(input) {
        ...
        match instruction {
            DO_INSTRUCTION => enabled = true,
            DONT_INSTRUCTION => enabled = false,
            _ => {
